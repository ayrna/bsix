...
class DeepSurv(BaseSurvival):
    ...
    """
    Deep survival model based on a neural-network Cox proportional hazards formulation.

    This implementation learns a nonlinear risk score from the covariates and
    optimizes a Cox-style partial log-likelihood instead of a direct regression
    objective. The inferred risk is later converted into survival and cumulative
    hazard curves through a Breslow baseline estimator.
    ...
    valid_data : dict, optional
        Validation data containing the keys ``x``, ``e`` and ``t``.
    ...
    Notes
    -----
    The model assumes a proportional hazards structure, so the hazard ratio
    between two observations is constant over time after conditioning on the
    covariates. The risk score is learned by the network and then mapped to
    survival curves through the Breslow estimator.

    Examples
    --------
    >>> from bsix.models.metodologies import DeepSurv
    >>> model = DeepSurv(number_inputs=10, hidden_layers=[32, 16], epochs=200, learn_rate=0.01)
    ...
    def _negative_log_likelihood(self, risk, t, e):
        ...
        -------
        torch.Tensor
            Scalar negative partial log-likelihood for the supplied risk score.
        """
    ...
    def _get_loss(self, x, e, t):
        ...
        """
        Compute the total training loss for the deep survival model.
    ...
    def fit(self, X_train, y_train, **kwargs):
        ...
        """
        Fit the DeepSurv model to the training data.
        ...
        y_train : structured array-like of shape (n_samples,)
            Target values with fields ``event`` and ``time``.
    ...
    def predict_survival_function(self, X, index, dataset, seed, plot=False):
        ...
        if plot:
            figure, ax = self._plot_survival_hazard_functions(self.survival_function, index, "DeepSurv", dataset, "Survival", seed)
            plt.show()
    ...
    def predict_cumulative_hazard_function(self, X, index, dataset, seed, plot=False):
        ...
        if plot:
            figure, ax = self._plot_survival_hazard_functions(self.cumulative_hazard_function, index, "DeepSurv", dataset, "CumulativeRisk", seed)
            plt.show()
    ...
    def calculate_xai(self, X, index, scaler, dataset, seed, feature_names, background=False, plot=False):
        ...
        if plot:
            figure, ax = BaseSurvival.plot_shap(self.shap_explainer, index, scaler, "DeepSurv", dataset, seed)
            plt.show()
