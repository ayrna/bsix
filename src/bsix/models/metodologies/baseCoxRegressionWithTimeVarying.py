...
class BaseCoxRegressionWithTimeVarying(BaseSurvival):
    ...
    def predict_survival_function(self, X, index, dataset, seed, plot=False):
        ...
        if plot:
            figure, ax = self._plot_survival_hazard_functions(self.survival_function, index, "BaseCoxRegressionWithTimeVarying", dataset, "Survival", seed)
            plt.show()
    ...
    def predict_cumulative_hazard_function(self, X, index, dataset, seed, plot=False):
        ...
        if plot:
            figure, ax = self._plot_survival_hazard_functions(self.cumulative_hazard_function, index, "BaseCoxRegressionWithTimeVarying", dataset, "CumulativeRisk", seed)
            plt.show()
    ...
    def calculate_xai(self, X, index, scaler, dataset, seed, feature_names, background=False, plot=False):
        ...
        if plot:
            figure, ax = BaseSurvival.plot_coefficients(self.coefficients, "BaseCoxRegressionWithTimeVarying", dataset, seed)
            figure, ax = BaseSurvival.plot_shap(self.shap_explainer, index, scaler, "BaseCoxRegressionWithTimeVarying", dataset, seed)
